...
contract TheSpirals is ERC721Enumerable, Ownable {
  ...
  function getUnmintedSpiralsByUser(address user)
    public
    view
    returns (uint256[] memory tokenIds)
  {
  ...
  function mintSpiral(uint256 tokenId) public {
    ...
    require(
      ...
      'Only token owner can mint their Spiral.'
    );
    ...
    returns (string memory)
  {
    ...
    bytes memory svgPartA = generateSVGPartA(
      tokenId,
      r,
      g,
      b,
    ...
    bytes memory svgPartB = generateSVGPartB(
      pathD,
      traits.strokeWidth,
      traits.stepDuration,
      traits.duration
  ...
  function generateSVGPartA(
    ...
    (
      string memory rHexCode,
      string memory gHexCode,
      string memory bHexCode
    ) = generateColorSpectrum(tokenId, r, g, b);
  ...
  function generatePathD(uint8 direction, uint8 spiralSize)
    internal
    view
    returns (string memory)
  {
    ...
    return string(abi.encodePacked(pathD, '0'));
  }
  ...
  function generateTraits(
    ...
    uint32 g,
    uint32 b
  ) internal view returns (SpiralTraits memory) {
